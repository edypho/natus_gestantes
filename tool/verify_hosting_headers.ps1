...
Assert-Header -Path '/flutter_bootstrap.js' -Name 'Cache-Control' -Expected 'no-cache, must-revalidate'
Assert-Header -Path '/firebase-messaging-sw.js' -Name 'Cache-Control' -Expected $noStore
...
Assert-Header -Path '/main.dart.js' -Name 'Cache-Control' -Expected 'no-cache, must-revalidate'
Assert-Header -Path '/downloads/natus-android-1.1.2.apk' -Name 'Cache-Control' -Expected $immutable
Assert-Header -Path '/downloads/natus-android-1.1.2.apk' -Name 'Content-Type' -Expected 'application/vnd.android.package-archive'
Assert-Header -Path '/downloads/natus-android-1.1.2.apk' -Name 'Content-Disposition' -Expected 'attachment; filename="natus-android-1.1.2.apk"'
