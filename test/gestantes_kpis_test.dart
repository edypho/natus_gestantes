...
void main() {
  group('statusGestanteNormalizado', () {
    test('normaliza variações com e sem acento', () {
      expect(statusGestanteNormalizado({'statusGestante': 'puerpera'}),
          'Puérpera');
      expect(statusGestanteNormalizado({'statusGestante': 'PUÉRPERA'}),
          'Puérpera');
      expect(statusGestanteNormalizado({'statusGestante': 'encerrado'}),
          'Encerrada');
      expect(statusGestanteNormalizado({'statusGestante': 'histórico'}),
          'Histórico');
    });

    test('ausente ou desconhecido vira Gestante', () {
      expect(statusGestanteNormalizado({}), 'Gestante');
      expect(statusGestanteNormalizado({'statusGestante': 'outro'}),
          'Gestante');
    });
  });
  ...
  group('converterDataDashboard', () {
    test('aceita ISO, brasileiro e brasileiro com hora', () {
      expect(converterDataDashboard('2026-05-22T17:38:36.210'),
    ...
    test('aceita formato Timestamp do Firestore', () {
      final d = converterDataDashboard('Timestamp(seconds=1750000000, nanoseconds=0)');
  ...
  group('KPIs', () {
    ...
    test('período por DPP: julho/2026 tem 2 (uma ativa, uma encerrada)', () {
      ...
      expect(
        contarEncerradasOuHistoricoNoPeriodoDpp(gestantes, 7, 2026),
        1,
      );
    });
